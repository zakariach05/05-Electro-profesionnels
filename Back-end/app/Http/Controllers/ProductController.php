...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('categories') || $request->has('category')) {
            ...
            // Slug mapping for backwards compatibility / English routes
            $slugMap = [
            ...
            if ($request->has('categories') && !empty($request->categories)) {
                $rawSlugs = explode(',', $request->categories);
                foreach ($rawSlugs as $slug) {
                    $categorySlugs[] = $slugMap[strtolower($slug)] ?? $slug;
                }
            }
            ...
            $categorySlugs = array_filter(array_unique($categorySlugs));
            ...
            if (!empty($categorySlugs)) {
                ...
                foreach ($categorySlugs as $slug) {
                    ...
                    if ($category) {
                        $allCategoryIds = array_merge($allCategoryIds, $this->getCategoryDescendants($category));
                    }
                }
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            ...
            'description' => 'nullable|string',
            'state' => 'nullable|string|in:neuf,occasion,reconditionné',
            'promo' => 'sometimes|integer|min:0|max:100',
            'is_featured' => 'nullable|boolean',

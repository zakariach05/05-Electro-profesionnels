...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        \Illuminate\Support\Facades\Log::info('--- NEW ORDER CREATION ---');
        \Illuminate\Support\Facades\Log::info('Payload:', $request->all());

        try {
            DB::beginTransaction();

            // Fetch real prices from database — never trust client input
            ...
            foreach ($request->items as $itemData) {
                ...
                $dbProduct = $dbProducts->get($itemData['id']);
                $realPrice = $dbProduct ? $dbProduct->price : 0;
                ...
                $sellerItems[$sellerId]['subtotal'] += $realPrice * $itemData['quantity'];
            }
            ...
            // Capture order data for the shutdown function (avoids timeout blocking the HTTP response)
            $orderId    = $order->id;
            $orderEmail = $order->customer_email;
            $adminEmail = 'chzakaria037@gmail.com';

            // Send emails AFTER the HTTP response is delivered — does NOT block CORS headers
            register_shutdown_function(function () use ($order, $adminEmail, $orderEmail) {
                try {
                    Mail::to($adminEmail)->send(new OrderAdminMail($order));
                    Mail::to($orderEmail)->send(new OrderConfirmationMail($order));
    ...
    {
        $request->validate(['status' => 'required|in:pending,processing,paid,completed,cancelled,delivered']);
